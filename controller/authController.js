...
export const protect = async (req, res, next) => {
  ...
  if (!decoded) throw new AppError('Invalid token. Please log in again.', 401);
...
export const forgotPassword = async (req, res, next) => {
  ...
  const otpExpires = '10m'; // 10 minutes
  ...
  try {
    ...
    throw new AppError('Error sending OTP. Please try again later.', 500);
  }
};
...
export const verifyOTP = async (req, res, next) => {
  const { otp } = req.body;
  if (!otp) {
    throw new AppError('Please provide the OTP', 400);
  }
  const hashedOtp = crypto.createHash('sha256').update(otp).digest('hex');
...
export const resetPassword = async (req, res, next) => {
  ...
  if (!newPassword || !confirmPassword) {
    throw new AppError('Please provide new password and confirm password', 400);
  }
...
export const updatePassword = async (req, res, next) => {
  ...
  if (!user) {
    throw new AppError('please log in to update your password', 404);
  }
